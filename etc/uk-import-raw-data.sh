...
if [ ! -f "$download/geo-data/postcode/GB_full.csv.zip" ]; then
  ...
  unzip -o GB_full.csv.zip
  mongoimport --host localhost --db geo_data_uk --collection PostalCode --drop --type tsv --file $download/geo-data/postcode/GB_full.csv -f countryCode,postalCode,placeName,adminName1,adminCode1,adminName2,adminCode2,adminName3,adminCode3,latitude,longitude,accuracy
  mongo localhost/geo_data_uk --eval "db.PostalCode.ensureIndex({postalCode:1});"
fi
